...
function renderScreenReaderInput(input, responses, updateResponses, clearError, decisionExplanation) {
  ...
  const legendContent = decisionExplanation ? `${label}. ${decisionExplanation}` : label;
  ...
  switch (type) {
    ...
    case 'select_option':
      return (
        <fieldset key={id}>
          <legend className="text-sm font-medium mb-3">{legendContent}</legend>
      ...
      <div className="flex flex-col gap-4">
        {inputs.map((input) => (
          <div key={input.id} className="flex flex-col gap-2">
            {renderScreenReaderInput(input, stepResponses, updateResponses, clearError, decisionExplanation)}
            {errors[input.id] && (
